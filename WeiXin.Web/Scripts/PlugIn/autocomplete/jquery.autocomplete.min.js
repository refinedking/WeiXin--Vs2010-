...
(function ($) {
    ...
    $.Autocompleter = function (input, options) {
        ...
        .bind(($.browser.opera ? "keypress" : "keydown") + ".autocomplete", function (event) {
            ...
            switch (event.keyCode) {
                case KEY.UP: event.preventDefault(); if (select.visible()) {
                    ...
                    break;
                case KEY.DOWN: event.preventDefault(); if (select.visible()) {
                        select.next();
                    } else {
                        onChange(0, true);
                    }
                    break;
                case KEY.PAGEUP: event.preventDefault(); if (select.visible()) {
                        select.pageUp();
                    } else {
                        onChange(0, true);
                    }
                    break;
                case KEY.PAGEDOWN: event.preventDefault(); if (select.visible()) {
                        select.pageDown();
                    } else {
                        onChange(0, true);
                    }
                ...
                default:
                    ...
                    timeout = setTimeout(onChange, options.delay); break;
            }
        })
        .focus(function () {
            hasFocus++;
        ...
        function selectCurrent() {
            ...
            if (!selected) return false; var v = selected.result; previousValue = v;
            if (options.multiple) {
        ...
        function autoFill(q, sValue) {
            if (options.autoFill && (lastWord($input.val()).toLowerCase() == q.toLowerCase()) && lastKeyPressCode != KEY.BACKSPACE) { $input.val($input.val() + sValue.substring(lastWord(previousValue).length)); $(input).selection(previousValue.length, previousValue.length + sValue.length); }
        };
        ...
        function receiveData(q, data) {
            if (data && data.length && hasFocus) { stopLoading(); select.display(data, q); autoFill(q, data[0].value); select.show(); } else {
                hideResultsNow();
            }
        };
    ...
    $.Autocompleter.defaults = { inputClass: "ac_input", resultsClass: "ac_results", loadingClass: "ac_loading", minChars: 1, delay: 400, matchCase: false, matchSubset: true, matchContains: false, cacheLength: 10, max: 100, mustMatch: false, extraParams: {}, selectFirst: true,
    ...
    $.Autocompleter.Cache = function (options) {
        ...
        function matchSubset(s, sub) {
            if (!options.matchCase) s = s.toLowerCase(); var i = s.indexOf(sub);
        ...
        function populate() {
            if (!options.data) return false; var stMatchSets = {}, nullData = 0; if (!options.url) options.cacheLength = 1; stMatchSets[""] = [];
            ...
            if (!options.url && options.matchContains) {
                ...
                for (var i = q.length - 1; i >= options.minChars; i--) {
                    var c = data[q.substr(0, i)]; if (c) {
                        var csub = [];
                        $.each(c, function (i, x) { if (matchSubset(x.value, q)) { csub[csub.length] = x; } }); return csub;
                    }
                }
            }
            return null;
    ...
    $.Autocompleter.Select = function (options, input, select, config) {
        ...
        function init() {
            ...
            }).mousedown(function () { config.mouseDownOnSelect = true; }).mouseup(function () {
            ...
            show: function () {
                ...
                if (options.scroll) {
                    ...
                    if ($.browser.msie && typeof document.body.style.maxHeight === "undefined") {
                        var listHeight = 0; listItems.each(function () { listHeight += this.offsetHeight; });
