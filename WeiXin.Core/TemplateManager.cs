...
namespace WeiXin.Core
{
    ...
    public class TemplateManager
    {
        ...
        object FuncNoHTML(object[] args)
        {
            ...
            return WeiXin.Core.Strings.NoHTML(args[0].ToString());
        }
        /// <summary>
        /// 字节截断，一个汉字算2个长度
        /// </summary>
        ...
        object FuncCutString(object[] args)
        {
            if (!CheckArgCount(2, "cutstring", args))
                return null;
            return WeiXin.Core.Strings.CutString(args[0].ToString(), Convert.ToInt32(args[1].ToString()));
        }
        /// <summary>
        /// HtmlEncode编码
        /// </summary>
        ...
        object FuncHTMLEncode(object[] args)
        {
            if (!CheckArgCount(1, "htmlencode", args))
                return null;
            return WeiXin.Core.Strings.HtmlEncode(args[0].ToString());
        }
        /// <summary>
        /// HtmlDecode解码
        /// </summary>
        ...
        object FuncHTMLDecode(object[] args)
        {
            if (!CheckArgCount(1, "htmldecode", args))
                return null;
            return WeiXin.Core.Strings.HtmlDecode(args[0].ToString());
        }
        /// <summary>
        /// UrlEncode编码
        /// </summary>
        ...
        protected object EvalBinaryExpression(BinaryExpression exp)
        {
            switch (exp.Operator)
            {
                case WeiXin.Core.Parser.TokenKind.OpOr:
                    {
                ...
                case WeiXin.Core.Parser.TokenKind.OpAnd:
                    {
                        object lhsValue = EvalExpression(exp.Lhs);
                        if (!Util.ToBool(lhsValue))
                            return false;
                ...
                case WeiXin.Core.Parser.TokenKind.OpIs:
                    {
                        object lhsValue = EvalExpression(exp.Lhs);
                        object rhsValue = EvalExpression(exp.Rhs);
                ...
                case WeiXin.Core.Parser.TokenKind.OpIsNot:
                    {
                        object lhsValue = EvalExpression(exp.Lhs);
                        object rhsValue = EvalExpression(exp.Rhs);
                ...
                case WeiXin.Core.Parser.TokenKind.OpGt:
                    {
                        object lhsValue = EvalExpression(exp.Lhs);
                        object rhsValue = EvalExpression(exp.Rhs);
                ...
                case WeiXin.Core.Parser.TokenKind.OpLt:
                    {
                        object lhsValue = EvalExpression(exp.Lhs);
                        object rhsValue = EvalExpression(exp.Rhs);
                ...
                case WeiXin.Core.Parser.TokenKind.OpGte:
                    {
                        object lhsValue = EvalExpression(exp.Lhs);
                        object rhsValue = EvalExpression(exp.Rhs);
                ...
                case WeiXin.Core.Parser.TokenKind.OpLte:
                    {
                        object lhsValue = EvalExpression(exp.Lhs);
                        object rhsValue = EvalExpression(exp.Rhs);
